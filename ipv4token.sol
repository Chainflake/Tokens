...
contract IPv4Token is Pausable, StandardToken, BlackList {
    string public name;
    ...
    // total tokens limit * decimals and + 1 - 4 294 967 296 - 592 709 120 (reserved)
    uint256 public constant _totalLimit =  3702258176000001;
    ...
    function transfer(address _to, uint256 _value) override public whenNotPaused returns (bool) {
        require(!isBlackListed[msg.sender], "IPv4Token: sender is blacklisted");
        if (deprecated) {
    ...
    function transferFrom(address _from, address _to, uint256 _value) public override whenNotPaused returns (bool) {
        require(!isBlackListed[_from], "IPv4Token: sender is blacklisted");
        if (deprecated) {
    ...
    function issue(uint256 amount) public onlyOwner {
        require(_totalSupply + amount > _totalSupply, "IPv4Token: overflow detected");
        require(balances[owner] + amount > balances[owner], "IPv4Token: overflow detected");
        require(_totalSupply + amount < _totalLimit, "IPv4Token: Resource limit reached");
    ...
    function redeem(uint256 amount) public onlyOwner {
        require(_totalSupply >= amount, "IPv4Token: insufficient supply");
        require(balances[owner] >= amount, "IPv4Token: insufficient balance");
